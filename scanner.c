...
static Token number(Scanner* scanner) {
    ...
    if (match(scanner, '.') && isDigit(peekNext(scanner))) {
        advance(scanner);
...
static Token string(Scanner* scanner) {
    while (!match(scanner, '"') && !isAtEnd(scanner)) {
        if (match(scanner, '\n')) scanner->line++;
        advance(scanner);
    }
...
Token scanToken(Scanner* scanner) {
    ...
    case '!': {
        ...
        if (match(scanner, '=')) {
            advance(scanner);
            token = makeToken(scanner, TOKEN_BANG_EQUAL);
        }
        else {
            token = makeToken(scanner, TOKEN_BANG);
        }
    ...
    case '=': {
        ...
        if (match(scanner, '=')) {
            advance(scanner);
            token = makeToken(scanner, TOKEN_EQUAL_EQUAL);
        }
        else {
            token = makeToken(scanner, TOKEN_EQUAL);
        }
    ...
    case '<': {
        ...
        if (match(scanner, '=')) {
            advance(scanner);
            token = makeToken(scanner, TOKEN_LESS_EQUAL);
        }
        else {
            token = makeToken(scanner, TOKEN_LESS);
        }
    ...
    case '>': {
        ...
        if (match(scanner, '=')) {
            advance(scanner);
            token = makeToken(scanner, TOKEN_GREATER_EQUAL);
        }
        else {
            token = makeToken(scanner, TOKEN_GREATER);
        }
